...
const canvaGet = createJsonGetter({
  base: BASE,
  label: "Canva",
  headers: envAuthHeaders({ env: "CANVA_TOKEN" }),
});
...
export const CANVA_TOOL_NAMES = ["canva_get", "canva_list", "canva_search"] as const;

export function registerCanvaTools(reg: ZodToolRegistrar): void {
  reg(
    "canva_list",
