/**
 * The real Fastmail JMAP client over `fetch`.
 *
 * Moved out of `server.ts` for the same reason as every other connector's
 * transport: an entry point that constructs its client at module scope cannot
 * be imported by a test, so none of the session discovery, capability checks or
...
/** The client the entry point uses. Credentials are read at construction. */
export function createFetchJmapClient(): JmapClient {
  return new FetchJmapClient();
}
