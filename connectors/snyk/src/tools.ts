...
/** Snyk wants the lowercase `token` scheme. Shared with `snykPost` below. */
const authHeader = envAuthHeaders({ env: "SNYK_TOKEN", scheme: "token" });

const snykGet = createJsonGetter({
  base: SNYK_API,
  label: "Snyk",
  headers: authHeader,
});
...
export function registerSnykTools(reg: ZodToolRegistrar): void {
  reg(
    "snyk_list",
